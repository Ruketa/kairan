...
export class ServerApp {
  ...
  public initServer(): void {
    ...
    const requestHandler = new RequestHandler(accountRepository, familyRepository);

    // routing
    server.get("/inhabitant", (req: http.IncomingMessage, res: http.ServerResponse) => {
      return requestHandler.inhabitants(req, res);
    });
    server.get("/families", (req: http.IncomingMessage, res: http.ServerResponse) =>{
